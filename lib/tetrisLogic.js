...
const TETROMINO_TYPES = {
  ...
  function getNextTetromino() {
    // Refill and shuffle the bag if it's empty
    if (tetrominoBag.length === 0) {
      tetrominoBag = shuffle(Object.keys(TETROMINO_TYPES));
    }
  
    // Get the next tetromino from the bag
    const nextType = tetrominoBag.pop();
    return { ...TETROMINO_TYPES[nextType], type: nextType };
  }
